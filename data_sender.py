...
class DataSender:
    def __init__(self, server_url=None):
        ...
        self.token = config.get(
            "token"
        )  # Token Sanctum, didapat saat register_device()
    ...
    def register_device(self, server_url):
        ...
        payload = {"device_info": system_info, "timestamp": datetime.now().isoformat()}
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                if not token:
                    ...
                    return {
                        "success": False,
                        "error": "Server tidak mengembalikan token. Pastikan server sudah menerapkan Sanctum.",
                    }
    ...
    def send_all_data(self, app_usage_data, browsing_history_data, location_data=None):
        results = {
            ...
            "browsing_history": self.send_browsing_history(
                browsing_history_data, location_data
            ),
        }
    ...
    def fetch_config(self):
        ...
        try:
            response = requests.get(
                config_url, headers=self._headers(with_json=False), timeout=10
            )
            if response.status_code == 401:
                return {
                    "success": False,
                    "error": "Unauthorized - token tidak valid, perlu register ulang",
                    "code": 401,
                }
            if response.status_code == 200:
                data = response.json()
            ...
            if "last_config" in current_config:
                return {
                    "success": True,
                    "config": current_config["last_config"],
                    "is_fallback": True,
                }
            return {"success": False, "error": str(e)}
    ...
    def send_all_data_with_screenshot(
        self,
        app_usage_data,
    ...
    def fetch_pending_actions(self):
        ...
        try:
            response = requests.get(
                actions_url, headers=self._headers(with_json=False), timeout=10
            )
            if response.status_code == 401:
                return {
                    "success": False,
    ...
    def acknowledge_action(self, action_id, status):
        ...
        try:
            response = requests.post(
                ack_url, json=payload, headers=self._headers(), timeout=10
            )
            if response.status_code == 200:
                return {"success": True}
            return {"success": False, "error": f"Status {response.status_code}"}
    ...
    def fetch_remote_status(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    "success": True,
                    "remote_active": data.get("remote_active", False),
                }
            return {"success": False, "error": f"Status {response.status_code}"}
        except Exception as e:
            return {"success": False, "error": str(e)}
